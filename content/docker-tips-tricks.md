...
- Need to stop a running container? `docker stop <container_name>`_Note that container name is different than tag name: container name is assigned on docker run, while tag name is assigned on docker build._
